...
Hz, S_1, S_2, dB_1, dB_2, R_1, R_2, fifty_1, fifty_2 = np.loadtxt("./data_simulated/s_para_files/VHF-145+.S2P", delimiter = "\t", unpack = True, comments = ["!", "#"], skiprows = 1)
...
Hz, S_1, S_2, dB_1, dB_2, R_1, R_2, fifty_1, fifty_2 = np.loadtxt("./data_simulated/s_para_files/VLF-575+___Plus25degC.S2P", delimiter = ",", unpack = True, comments = ["!", "#"], skiprows = 1)
...
data_time, data_trace = np.loadtxt("./data_raw/2021_08_02_psuedo_air_to_air_with_46db_att_Ch1.csv",
                                delimiter=",", skiprows = 6, usecols=(3, 4), unpack = True)
...
#plt.plot(master_freqs * 1e3, dbm(numc_signal), color = 'purple', label = "NuRadio Sim - No Reflection")
#plt.plot(master_freqs * 1e3, dbm(mine_signal), color = 'red', label = "xFDTD Sim - No Reflection")
...
plt.savefig("./plots/A04_calc_systematics_air_normalization_reflections.png", dpi = 300)
...
np.savez("./data_processed/A04_calc_systematic_air_normalization.npz", freq = master_freqs, ratio = np.power(10.0, (dbm(mine_signal_) - dbm(mine_signal)) / 20.0))
